...
st.title('Inshorts News API')
st.header('Get News from Inshorts')
st.subheader('Select a Category')
category = st.selectbox(
    'Category',
    ('all', 'india', 'business', 'politics', 'sports', 'technology', 'startup', 'entertainment', 'hatke', 'science', 'automobile')
)
news = getNews(category)
if news['success']:
    for entry in news['data']:
        st.markdown('---')
        st.image(entry['imageUrl'])
        st.markdown(f'# {entry["title"]}')
        st.markdown(f'## {entry["content"]}')
        st.markdown(f'### Author: {entry["author"]}')
        st.markdown(f'### Published At: {entry["date"]} {entry["time"]}')
        st.markdown(f'[Read More]({entry["readMoreUrl"]})')
else:
    st.markdown(f'## {news["error"]}')
